...
class _BriefCard extends StatelessWidget {
        ...
        children: [
          ...
          if (insights.isEmpty)
            ...
            for (var i = 0; i < insights.length; i++) ...[
              if (i > 0) const SizedBox(height: TWSpace.lg),
              _InsightBlock(index: i + 1, insight: insights[i]),
            ],
          const SizedBox(height: TWSpace.md),
          Row(
            children: [
              Icon(Icons.auto_awesome,
                  size: 11, color: TWColors.textTertiary),
              const SizedBox(width: 5),
              Text('Generated by Wizz',
                  style: TWType.caption
  ...
  List<_BriefInsight> _insights() {
    ...
    // 1) Market Outlook — headline + top opportunity reference.
    ...
    out.add(_BriefInsight(
      ...
      ticker: o?.symbol,
      tickerMeta: o != null ? 'Score ${o.score.toStringAsFixed(0)}' : null,
      tickerLabel: o != null ? 'Top opportunity' : null,
    ));

    // 2) Strongest Signal — the top opportunity, broken into chips.
    if (o != null) {
      out.add(_BriefInsight(
        title: 'Strongest Signal',
        ticker: o.symbol,
        tickerMeta: 'Score ${o.score.toStringAsFixed(0)}',
        chips: _signalChips(o),
      ));
    }
...
/// Immutable view-model for one Morning Brief insight block (presentation
...
class _BriefInsight {
  ...
  final String? tickerLabel;
  final bool tickerIsAccent;
  final List<String> chips;
}

/// A single numbered briefing block: index circle + structured content.
class _InsightBlock extends StatelessWidget {
  const _InsightBlock({required this.index, required this.insight});
  final int index;
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color: TWColors.accent.withValues(alpha: 0.4), width: 1),
          ),
          child: Text('$index',
          ...
          child: Column(
            ...
            children: [
              ...
              if (i.body != null) ...[
                ...
                Text(i.body!,
                    style: TWType.bodySm.copyWith(
                        color: TWColors.textSecondary, height: 1.3)),
              ],
              if (i.ticker != null) ...[
              ...
              if (i.chips.isNotEmpty) ...[
                const SizedBox(height: TWSpace.sm),
                Wrap(
                  spacing: TWSpace.sm,
                  runSpacing: 6,
                  children: [for (final c in i.chips) TWTagChip(label: c)],
                ),
              ],
            ],
          ),
...
class _PortfolioCard extends StatelessWidget {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Value block — label + loud number + total return.
            ...
            const SizedBox(height: 16),
            // Inset performance card — Today's P/L.
            _todayCard(todayPnl, todayPct, a.currency),
  ...
  Widget _todayCard(double v, double pct, String currency) {
    final up = v >= 0;
    return Container(
      width: double.infinity,
